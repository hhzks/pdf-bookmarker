...
| LLM alone | 100% | 0.7637 | 0.7546 |
| `--labeler` | 0% | 0.8009 | **0.8928** |
| `--labeler` + auto mode (default) | 38% | 0.8211 | 0.8855 |
| `--labeler --llm` | 100% | **0.8312** | 0.8755 |

The LLM row is the shipped GGUF, replayed from predictions generated by
`training/predict.py --gguf` so the whole table is one model measured one way.

**On titles the heading model and the LLM are tied** — paired per document,
+0.0372 for the model with a 95% CI of [−0.0133, +0.0876], 31 documents better
against 31. Its advantage is everything else: hierarchy (+0.1382, CI [+0.0715,
+0.2122], 21 better against 7), exact physical pages by construction, no API
key, and milliseconds per document against minutes.

The two detectors miss different headings — the model cannot name a heading
that is not a line of text, the LLM reconstructs wrapped and merged ones. They
propose 3665 distinct titles between them and agree on only 30.8%, with 34.6%
from the model alone and 34.7% from the LLM alone. That is why the last two
rows merge the outlines rather than letting one replace the other.

The two level-accuracy figures answer different questions. **0.8928 is the
heading model alone**; **0.8855 is end-to-end at the default routing**, where
...
**0.5 headings per page or fewer**, which routes 38% of documents.
`--llm-density` moves the threshold; `0` never escalates, `1.5` escalates on
nearly everything.

**The threshold is a cost control, not a quality ceiling.** Quality rises the
more you route, and the rise is real rather than noise — paired against the
labeler alone, +0.0202 at the default (CI [+0.0000, +0.0418]), +0.0270 at
`1.0`, +0.0309 at `1.5`. Routing beyond the default is still worth something
measurable: `1.5` over `0.5` is +0.0107, CI [+0.0037, +0.0184], 21 documents
better against 7. The default keeps roughly two thirds of the available gain
for 38% of the calls, so raise it if quality matters more than compute.
...
On a CPU host the local GGUF costs minutes per document for +0.0202 title F1 at
the default routing (+0.0303 with `--llm`) — real, but not a trade worth making
by default when the host is CPU-only and the caller is waiting. Setting
`VERIFICATION_MODEL`
